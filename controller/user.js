...
const stripe = require("stripe")(
  "sk_test_51MEcpGLCNC3Hn0osJ7je3vOMv6f9KK8zB4GOeZ1v5FSptPQZMGfbyjvB4rc350FKbfOO7ek5LECz75dMNaz8RNvU00VRccAgDr"
);
const paypal = require("paypal-rest-sdk");
const product = require("../model/product");
paypal.configure({
  mode: "sandbox", //sandbox or live
  client_id: "", // ARFpVeDTsGuRYRzndTcPgCQnFbq9TZn2oov0M3riZAGbefxnmvAdgWQz2MUKvlVchBdS5IKMPRO31zJN
  client_secret: "", // EKYoVUC-kfNxo_LB7Ur-LDsmVMhF2JVe29Xs2r7l2IIQFnp248QHC66uFdqGKc0WAk6rF5J6vObBv0Oj
});
...
exports.signUp = async (req, res) => {
  ...
  if (type == "0") {
    ...
    User.findOne({ google_uid: body.google_uid, isDeleted: { $ne: 1 } }).then(
      (found) => {
        if (found) {
          foundUser = found;
          const token = jwt.sign(
    ...
    User.findOne({
      facebook_uid: body.facebook_uid,
      isDeleted: { $ne: 1 },
    }).then((found) => {
      if (found) {
        foundUser = found;
    ...
    User.findOne({ kakao_uid: body.kakao_uid, isDeleted: { $ne: 1 } }).then(
      (found) => {
        if (found) {
          foundUser = found;
          const token = jwt.sign(
...
exports.update = async (req, res) => {
  try {
    const user = await User.findOne(
      { _id: req.body.id, isDeleted: { $ne: 1 } },
      { __v: 0, createdAt: 0, updatedAt: 0 }
    );
...
exports.delete = async (req, res) => {
  try {
    const user = await User.findOne({ _id: req.user.id }, { isDeleted: 1 });
...
exports.stripe = async (req, res) => {
  ...
  const products = await product.find(
    { _id: { $in: productIds } },
    { price: 1, title: 1, images: 1, description: 1 }
  );
  ...
  var create_payment_json = {
    ...
    redirect_urls: {
      return_url: `http://localhost:5000/api/user/success?total=${
        totalPrice + 10
      }&useremail=${req.params.useremail}&membershipId=${
  ...
  paypal.payment.execute(
    ...
    function (error, payment) {
      if (error) {
        ...
        res.redirect(
          `http://localhost:5000/api/user/afterpurchase/${membershipId}/${userEmail}`
        );
      }
    }
  );
